...
class Actor(nn.Module):
    def __init__(self, action_dim, max_action, conv_layers, conv_output_dim, initialize, img_width, stack):
...
class Critic(nn.Module):
    def __init__(self, action_dim, conv_layers, conv_output_dim, initialize, img_width, stack):
...
class TD3PixelsShared(object):
    def __init__(self, state_dim, action_dim, max_action, arch="mine", initialize=True, img_width=128, stack=4, ddpglr=False, lr_schedule=False):
        ...
        self.actor = Actor(action_dim, max_action, self.conv_layers, conv_output_dim, initialize, img_width, stack).to(device)
        self.actor_target = Actor(action_dim, max_action, self.target_conv_layers, conv_output_dim, initialize, img_width, stack).to(device)
        ...
        self.critic = Critic(action_dim, self.conv_layers, conv_output_dim, initialize, img_width, stack).to(device)
        self.critic_target = Critic(action_dim, self.target_conv_layers, conv_output_dim, initialize, img_width, stack).to(device)
