...
import OurDDPG
import DDPG
from pixel_wrapper import PixelObservationWrapper, IMG_SIZE
...
def render_policy(policy, log_dir, total_timesteps, eval_episodes=5):
    ...
    for episode in range(eval_episodes):
        obs = env.reset()
        policy.reset()
        frames.append(env.render(mode='rgb_array', width=IMG_SIZE, height=IMG_SIZE))
        done = False
        while not done:
            action = policy.select_action(obs)
            obs, reward, done, _ = env.step(action)
            frame = env.render(mode='rgb_array', width=IMG_SIZE, height=IMG_SIZE)
...
if __name__ == "__main__":
    ...
    if args.policy_name == "TD3": 
        policy = TD3_pixels.TD3Pixels(state_dim, action_dim, max_action, 
                arch=args.arch, initialize=args.init, img_width=IMG_SIZE)
